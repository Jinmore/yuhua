...
class index {
 ...
 public function search()
 {
	 if($_GET['se1'] && $_GET['se2'])
	 {
		...
		//  echo $id1,$id2,$id3;
		//  实例化模型
		$model=pc_base::load_model("qualifications_model");
		$where= "zizhi = '$id3'";
		$data=$model->get_one($where);
		$id=$data['id'];
		$res="select * from ld_qualifications_data where id= '$id'";
		$result=mysql_query($res);
		$content=mysql_fetch_array($result);
		echo json_encode(array("title"=>$data['title'],"content"=>$content['content']));
	...
	public function send()
	{
		if($_POST)
		{
			...
			$data['datetime']=time();
			$data['ip']=ip();
			$model=pc_base::load_model('form_message_model');
